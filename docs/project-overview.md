...
- **복사(duplicate)로 만든 상품의 기본값은 `private`.** 편집 후 공개범위를 안 바꾸고 저장하면 공개 주소가 404가 된다. (2026-07-29 230번 사건의 원인)
...
1. **복사본이 `private`로 남아 안 보임** — 복사 → 편집 → 저장까지 다 되지만 공개범위를 안 바꾸면 내용이 안 보인다. 편집 화면에 경고를 띄우고, 공개 주소는 404 대신 "비공개" 안내 화면을 보여주도록 수정됨. (2026-07-29)
