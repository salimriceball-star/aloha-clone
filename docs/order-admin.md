...
- 주문완료 페이지는 `/checkout/order-received/[orderId]?key=...` 에서 UUID 주문 ID와 144-bit 무작위 `key`가 모두 일치할 때만 서버 기록을 표시한다.
- 관리자 화면은 최신 주문부터 카드 목록으로 보여 주며 이메일, 연락처, 메모, 상품 라인아이템을 함께 확인한다.
- BrowserOS QA 스크립트: `/home/ahn/aloha/scripts/browseros-order-admin-qa.ts`
